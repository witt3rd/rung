...
### 4.3 Carry immutability

The carry is supposed to be read-only witness data. The `carry { ... }` declaration in the ladder syntax names this intent. But the generated code makes `carry: Carry` a `pub` field — transitions can mutate it.

**Why this gap exists:** Rust doesn't have a "read-only field" construct. The closest approximation is a getter method that returns `&Carry`, but then the transition body can't move data *out* of the carry (which it sometimes needs to do — e.g., extracting the `task_id` as a reference for logging).

**Path to close:** The macro could generate the carry as a separate struct with only immutable accessors, passed by `&Carry` to transitions. Fields that need to move get `Clone`; everything else stays behind `&`. The tradeoff: some carry data legitimately needs to be extracted (e.g., a `correlation_key: Uuid` passed to a downstream system). A `carry` block annotation like `carry { task_id: String, +transfer correlation_key: Uuid }` could mark fields that are consumed by the first transition that reads them.
...
| Carry read-only | — | — | ✗ Convention only |
