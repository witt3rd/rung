...
on proposition slug, so it survives every renumbering (see `_props.py`).
    ...
    ./_ledger.py check    exit 1 if coverage, slugs, or cited tests are wrong
"""
...
CURATED = {
    ...
    "non-identity-before-dispatch": (
        "expressible",
        "The filter is set operations over declared predicates (10.22). rung enforces "
        "*that the token was constructed*, never that the body computed the set "
    ...
    "reproposal-carries-the-chain": (
        ...
        "— which is why re-entry must not use a guarded edge (12.5).",
        "—",
    ),
    ...
    "guarded-reentry-is-eviction": (
        ...
        "progress and is therefore an eviction rule (12.2). "
        "CONSTRAINT: a continue arm's target rung is built inline by `dispose`, i.e. "
        "by the judge, so that rung's payload must be classification-only (7.42).",
...
`./_ledger.py check`, which fails if a proposition is missing, a slug is
unknown, or an `enforced` row cites a file that does not exist.
...
def check():
    if not LEDGER.exists():
        print("  conformance.md does not exist — run ./_ledger.py gen", file=sys.stderr)
        return 1
    errs = []
