...
| [8.21](rung-het-props.md#het-declares-no-worth-law) | `het-declares-no-worth-law` | `decidable` | **The α cut, given teeth.** This row and [3.31](rung-het-props.md#ordering-is-hetopts) were `out-of-scope` by default and never inspected — correctly, while nothing in the workspace declared a cost tier or an ε for a worth-law to be built out of. `rung-std::principals` declares both, so the refusal is now a property a run can check: the cited test reads every line of Rust in all four crates that names a cost tier or an ε and fails on any that also sorts, compares, ranks or takes an extremum. It reads attribute lines above a hit as well, so `#[derive(.., Ord)]` on `CostTier` is caught although the derive names no cost of its own — that derive is the cheapest possible crossing of the cut and it is the mutation this test exists to see. `CostTier` and `Epsilon` independently carry no `Ord`, no `PartialOrd` and no accessor, so the minimal-judge rule of [8.22](rung-het-props.md#v-applies-to-conforming-sets) has neither a comparison nor a value to read.  The scan carries exactly one licensed exception: `rung-het/src/hetopt.rs`, the HetOpt layer itself — [3.31](rung-het-props.md#ordering-is-hetopts) is where the ordering belongs, and [8.22](rung-het-props.md#v-applies-to-conforming-sets)'s three valuations (minimal-judge, minimal-author, candidate ranking) are implemented there over the [`Priced`] extension trait, one axis over the kernel `Principal` interface, so [3.23](rung-het-props.md#nothing-further-required) still holds. The cut is drawn **at** valuation: the module that is valuation is where the order lives, and the guard refuses it everywhere else. The extension is reached only through a re-indexing wrapper ([`ValuedPool`]), which runs Het's filters whole and first — [3.55](rung-het-props.md#non-identity-not-deferrable) survives the extension by construction. | `rung-std/tests/principals_theory.rs::nothing_in_the_workspace_orders_by_cost_or_epsilon` |
...
**`rung-driver/tests/panel.rs`** — 3 unclaimed

- `a_consensus_panel_closes_and_records_every_judge`
...
**`rung-std/tests/questions_theory.rs`** — 22 unclaimed
...
- `rewriting_repairs_to_conformity_and_verifies`
