...
#[test]
fn the_sixteen_questions_are_read_from_disk() {
    let r = load();
    let ids: Vec<&str> = r.questions.iter().map(|q| q.id.as_str()).collect();
    assert_eq!(
        ids,
        [
            "q1", "q10", "q11", "q12", "q13", "q14", "q15", "q16", "q2", "q3", "q4", "q5", "q6",
            "q7", "q8", "q9"
        ],
        "questions/ holds sixteen questions; an audit over zero of them proves nothing"
    );
}
