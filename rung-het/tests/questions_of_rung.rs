...
//! decidable sentence is evaluated against all of them. If a sentence is
...
//! genuinely rung's: a [`Scheme`], a directory, the question files, and the results.
//!
...
#[test]
fn the_nineteen_questions_are_read_from_disk() {
    let r = load();
    let ids: Vec<&str> = r.questions.iter().map(|q| q.id.as_str()).collect();
    assert_eq!(
        ids,
        [
            "q1", "q10", "q11", "q12", "q13", "q14", "q15", "q16", "q17", "q18", "q19", "q2", "q3",
            "q4", "q5", "q6", "q7", "q8", "q9"
        ],
        "questions/ holds nineteen questions; an audit over zero of them proves nothing"
    );
}

#[test]
fn every_per_question_decidable_sentence_holds_over_all_questions() {
    let r = load();
