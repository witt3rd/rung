...
description: >-
  ...
  self), when you want a recoverable event log instead of a growing last-words file,
...
A single growing last-words file fails: it's great for context-loading but it doesn't scale —
it bloats, conflates your notes with other people's messages, and merges into chaos when two
agents write it at once. That file is retired and taboo in this repo. The fix is to stop
maintaining one mutable state file and make the **history itself** the state.
